//Demo by Olivier Potgieter.
//Simple Showcase of C++ skillset part 1 of series.
//This was an old project I had done while learning C++, reflecthing on this project there is various things I would alter now. 
...
float* ghost1 = new float[3] {10.5, 8.5, 1.0}; //coordinates and direction of first ghost
float* ghost2 = new float[3] {13.5, 1.5, 2.0}; //coordinates and direction of second ghost
float* ghost3 = new float[3] {4.5, 6.5, 3.0}; //coordinates and direction of third ghost
float* ghost4 = new float[3] {2.5, 13.5, 4.0}; //coordinates and direction of fourth ghost
...
void drawghost(float positionX, float positionY, float r, float g, float b) {
	int x, y;
	glBegin(GL_LINES);
	glColor3f(r, g, b);
	//draw the head
...
void updateghost(float* ghost, int id) {
	//find the current position of the ghost
	int x1Quadrant = (int)((ghost[0] - (2 / squareSize)) - (16.0 * cos(360 * M_PI / 180.0)) / squareSize);
	int x2Quadrant = (int)((ghost[0] + (2 / squareSize)) + (16.0 * cos(360 * M_PI / 180.0)) / squareSize);
	int y1Quadrant = (int)((ghost[1] - (2 / squareSize)) - (16.0 * cos(360 * M_PI / 180.0)) / squareSize);
	int y2Quadrant = (int)((ghost[1] + (2 / squareSize)) + (16.0 * cos(360 * M_PI / 180.0)) / squareSize);
	//move him acording to its direction until he hits an obstacle
	switch ((int)ghost[2]) {
	case 1:
		if (!bitmap.at(x1Quadrant).at((int)ghost[1])) {
			ghost[0] -= 2 / squareSize / 4;
		}
		else {
			int current = ghost[2];
			do {
				ghost[2] = (rand() % 4) + 1;
			} while (current == (int)ghost[2]);
		}
		break;
	case 2:
		if (!bitmap.at(x2Quadrant).at((int)ghost[1])) {
			ghost[0] += 2 / squareSize / 4;
		}
		else {
			int current = ghost[2];
		...
		break;
	case 3:
		if (!bitmap.at((int)ghost[0]).at(y1Quadrant)) {
			ghost[1] -= 2 / squareSize / 4;
		}
		else {
			int current = ghost[2];
			do {
		...
		break;
	case 4:
		if (!bitmap.at((int)ghost[0]).at(y2Quadrant)) {
			ghost[1] += 2 / squareSize / 4;
		}
		else {
			int current = ghost[2];
...
void resetGame() {
	...
	yIncrement = 0;
	rotation = 0;
	ghost1 = new float[3] {10.5, 8.5, 1.0};
	ghost2 = new float[3] {13.5, 1.5, 2.0};
	ghost3 = new float[3] {4.5, 6.5, 3.0};
	ghost4 = new float[3] {2.5, 13.5, 4.0};
	points = 0;
	for (int i = 0; i < 256; i++) {
		keyStates[i] = false;
	}
...
void gameOver() {
	...
	int ghost1X = (int)(ghost1[0]);
	int ghost1Y = (int)(ghost1[1]);
	int ghost2X = (int)(ghost2[0]);
	int ghost2Y = (int)(ghost2[1]);
	int ghost3X = (int)(ghost3[0]);
	int ghost3Y = (int)(ghost3[1]);
	int ghost4X = (int)(ghost4[0]);
	int ghost4Y = (int)(ghost4[1]);
...
void resultsDisplay() {
	if (points == 106) {
		...
		message = "Wow I can't believe someone actaully won";
		glColor3f(1, 1, 1);
...
void welcomeScreen() {
	glClearColor(0.086, 0.678, 0, 1.0);
	const char* message = "=====================================";
	glRasterPos2f(150, 200);
	while (*message)
		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, *message++);
	message = "PACMAN Demo - by Olivier Potgieter";
	glColor3f(1, 1, 1);
...
void display() {
	...
	if (replay) {
		if (!over) {
			...
			updateghost(ghost1, 1);
			updateghost(ghost2, 2);
			updateghost(ghost3, 3);
			updateghost(ghost4, 4);
			drawghost(ghost1[0], ghost1[1], 0.0, 1.0, 1.0); //cyan
			drawghost(ghost2[0], ghost2[1], 1.0, 0.0, 0.0); //red
			drawghost(ghost3[0], ghost3[1], 1.0, 0.0, 0.6); //magenta
			drawghost(ghost4[0], ghost4[1], 1.0, 1.0, 1.0); //white
		}
		else {
			resultsDisplay();
		}
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("PACMAN Demo - by Olivier Potgieter");
